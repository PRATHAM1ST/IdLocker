		...
		async (
			...
			setTimeout(() => {
				setItems((currentItems) => {
					vaultStorage
						.saveVault({ version: 1, items: currentItems })
						.catch(() => {
							logger.debug(
								"Item added to memory but not persisted"
							);
						});
					return currentItems;
				});
			...
			const updatedItem: VaultItem = {
				...
				images:
					updates.images !== undefined
						? updates.images
						: existingItem.images, // Include legacy images
				assetRefs:
					updates.assetRefs !== undefined
						? updates.assetRefs
						: existingItem.assetRefs, // Include asset refs
			...
			setTimeout(() => {
				setItems((currentItems) => {
					vaultStorage
						...
						.catch(() => {
							logger.debug(
								"Item updated in memory but not persisted"
							);
						});
					return currentItems;
				});
			...
			setTimeout(() => {
				setItems((currentItems) => {
					vaultStorage
						...
						.catch(() => {
							logger.debug(
								"Item deleted from memory but not persisted"
							);
						});
					return currentItems;
				});
